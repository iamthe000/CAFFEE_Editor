...
<div align="center">
【祝】PyPI 全期間でのダウンロードが2万を超えました🎉<br>
...
読み方やカタカナ表記は、キャフィーもしくはキャッフィーです☕️
...
## 🤝 コントリビューション / ブランチ戦略
本プロジェクトへの貢献を歓迎します！エディタの安定性を保ち、PyPIへの誤った自動デプロイを防ぐため、ブランチを分けて開発を行っています。

**`main` ブランチへの直接のプッシュは行わないでください。**

* **`main`**: 常にリリース可能な安定版のブランチです（ここでタグを打つとPyPIへ自動デプロイされます）。
* **`develop`**: 開発とテストのための統合ブランチです。新機能やバグ修正は、まずここにマージされます。

**開発・参加の手順:**
1. このリポジトリをフォークします。
2. **`develop`** ブランチをベースにして、作業用の新しいブランチを作成します（例: `feature/add-new-theme` や `fix/typo`）。
3. コードに変更を加え、コミットします。
4. 自分のフォークしたリポジトリにブランチをプッシュします。
5. 本リポジトリの **`develop`** ブランチをターゲットにして、プルリクエスト（PR）を作成してください。
